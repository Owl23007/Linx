...
public interface ChatMessageMapper extends BaseMapper<ChatMessageEntity> {
    ...
    @Select("SELECT * FROM chat_messages WHERE is_deleted = 0 AND " +
    ...
    @Select("SELECT COUNT(*) FROM chat_messages WHERE is_deleted = 0 AND " +
            "receiver_id = #{userId} AND sender_id = #{otherUserId} AND is_read = 0")
    long countUnreadPrivateMessages(@Param("userId") Long userId,
                                    @Param("otherUserId") Long otherUserId);

    @Select("SELECT COUNT(*) FROM chat_messages WHERE is_deleted = 0 AND " +
            "receiver_id = #{userId} AND is_read = 0")
    long countAllUnreadMessages(@Param("userId") Long userId);

    @org.apache.ibatis.annotations.Update("UPDATE chat_messages SET is_read = 1, read_at = #{readAt} WHERE message_id = #{messageId}")
    int markAsRead(@Param("messageId") String messageId, @Param("readAt") LocalDateTime readAt);

    @org.apache.ibatis.annotations.Update("UPDATE chat_messages SET is_read = 1, read_at = #{readAt} WHERE receiver_id = #{userId} AND sender_id = #{otherUserId} AND is_read = 0")
    ...
    @org.apache.ibatis.annotations.Update("UPDATE chat_messages SET is_deleted = 1 WHERE message_id = #{messageId}")
    ...
    @Select("SELECT * FROM chat_messages WHERE id IN (" +
            "SELECT MAX(id) FROM chat_messages WHERE is_deleted = 0 AND (sender_id = #{userId} OR receiver_id = #{userId}) " +
