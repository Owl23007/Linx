<template>
  <aside class="flex h-full flex-col items-center justify-between bg-slate-50/90 px-2 py-3">
    <div class="flex flex-col items-center gap-3">
      <button type="button"
        ...
        <el-avatar :size="40" :src="avatarUrl"
          class="bg-slate-950 text-[13px] font-semibold shadow-lg shadow-slate-300/55">
          {{ userInitials }}
        </el-avatar>
      </button>

      <nav class="flex flex-col gap-1.5">
        <button v-for="item in items" :key="item.key" type="button"
          ...
          <span v-if="item.badge"
            class="absolute -right-2 -top-2 rounded-full bg-rose-500 px-1.5 py-0.5 text-[8px] font-semibold leading-none text-white">
            {{ item.badge }}
          </span>
        </button>
      </nav>
    ...
    <div class="flex flex-col items-center gap-2">
      <button type="button"
        class="flex h-10 w-10 items-center justify-center rounded-[18px] transition-all duration-200" :class="settingsActive
